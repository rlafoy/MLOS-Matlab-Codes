function camera_calibration_01;
...
% points on a black background (if true).  If the variable is fales, then
...
image_filenames=cell(camera_number,plane_number);
% This initializes the calibration plate orientation cell array
plate_orientation=cell(camera_number,plane_number);
...
for ii=1:camera_number;
    for jj=1:plane_number;
        % This creates the filename string
%         filename_string=['/mnt/current_storage/Projects/Tomo_PIV/101107_Vortex_Ring_Processing/Camera_Position_01/Calibration_01/z_',sprintf('%02d',jj-1),'/cam_',num2str(ii),'/cam_',num2str(ii),'000001.tif'];
        filename_string=['/mnt/current_storage/Projects2/Tomo_PIV/101107_Vortex_Ring_Processing/Camera_Position_01/Calibration_01/cam_',num2str(ii),'/z_',sprintf('%02d',jj-1),'/cam_',num2str(ii),'000001.tif'];
        image_filenames{ii,jj}=filename_string;
        % This saves the calibration plate orientation data into the cell
        % array
        if ii==1;
...
calibration_data.filename_save=filename_save;

% This initializes the coordinate data cell array in the structure
calibration_data.coordinate_data=cell(camera_number,plane_number);

% This creates the calibration function
generate_calibration(calibration_data);



function generate_calibration(calibration_data);
...
plane_number=calibration_data.plane_number;
...
calibration_plane_data=cell(camera_number,plane_number);
% This iterates through the cameras
for ii=1:1;%camera_number;
    % This iterates through the calibration planes
    for jj=1:1;%plane_number;
...
for ii=1:camera_number;
    % This iterates through the calibration planes
    for jj=1:plane_number;
        % This calculates the image and world coordinates of the grid
        % points
...
% calibration_data.x_image_full=cell(4,1);
% calibration_data.y_image_full=cell(4,1);
% calibration_data.x_world_full=cell(4,1);
% calibration_data.y_world_full=cell(4,1);
% calibration_data.z_world_full=cell(4,1);
...
for ii=1:plate_level_number;
    ...
    figure_string=['Camera ',num2str(current_camera_number),' Plane ',num2str(current_plane_number),': Locating the n = ',num2str(ii),' grid system.'];
...
for ii=1:plate_level_number;
    % This is the figure string to display
    figure_string=['Camera ',num2str(current_camera_number),' Plane ',num2str(current_plane_number),': Select the n = ',num2str(ii),' grid system.'];
    % This sets the figure title to tell the use to select the current grid plane
    set(1,'Name',figure_string);
